...
interface CachedFood {
  ...
  fat: number
  fiber: number
  source: 'usda' | 'openfoodfacts' | 'indian' | 'local'
  cachedAt: admin.firestore.FieldValue | number
}
  ...
  async (req) => {
    ...
    try {
      const snap = await db.collection('foodCache')
        ...
        .get()
      if (!snap.empty) {
        snap.docs.forEach(d => results.push(d.data() as CachedFood))
        if (results.length >= 5) return { foods: results, source: 'cache' }
      }
    ...
    try {
      ...
      indianSnap.docs.forEach(d => {
        const data = d.data() as CachedFood
        if (!results.find(r => r.fdcId === data.fdcId)) results.push(data)
      })
    } catch { /* continue */ }
    ...
    if (results.length < 8) {
      try {
        ...
        const res = await fetch(url, { headers: { 'User-Agent': 'FitvoryaAI/1.0' } })
        if (res.ok) {
...
export const searchExercises = onCall(
  ...
  async (req) => {
    ...
    try {
      let fsQuery = db.collection('exerciseCache') as FirebaseFirestore.Query
      if (bodyPart) {
...
export const seedIndianFoods = onRequest(
  { cors: false },
  async (_req, res) => {
...
function parseUSDA(f: USDAFood): Omit<CachedFood, 'source' | 'cachedAt'> {
...
function parseOFF(p: OFFProduct): (Omit<CachedFood, 'source' | 'cachedAt'>) | null {
...
function parseExerciseDB(e: ExerciseDBItem): CachedExercise {
  return {
    id:               e.id,
...
const INDIAN_FOODS: Omit<CachedFood, 'source' | 'cachedAt' | 'searchTerms'>[] = [
  // South Indian
