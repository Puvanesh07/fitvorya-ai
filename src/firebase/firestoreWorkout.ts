import {
  ...
  setDoc, deleteDoc, serverTimestamp, query,
  orderBy, limit, where,
...
export async function getWorkoutSessions(uid: string): Promise<WorkoutSession[]> {
  const ref  = collection(db, 'users', uid, 'workouts')
  // Composite index: startedAt DESC — defined in firestore.indexes.json
...
export async function getSessionsForHeatmap(
  ...
  // Single-field range on `date` — Firestore auto-index handles this.
  // Capped at 365 to avoid reading more than one year of docs.
  const q    = query(ref, where('date', '>=', cutoffStr), orderBy('date', 'asc'), limit(365))
  const snap = await getDocs(q)
  const map  = new Map<string, number>()
  snap.docs.forEach(d => {
    const date = (d.data() as WorkoutSession).date
    if (typeof date === 'string') map.set(date, (map.get(date) ?? 0) + 1)
  })
...
export async function getPersonalRecords(uid: string): Promise<PersonalRecord[]> {
  ...
  // Small collection (one doc per exercise) — no limit needed
...
export async function savePersonalRecord(uid: string, pr: PersonalRecord): Promise<void> {
  await setDoc(doc(db, 'users', uid, 'personalRecords', pr.exerciseId), pr)
}
